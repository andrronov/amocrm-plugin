	...
	return function () {
		...
		this.callbacks = {
			render: function () {
				...
				let dealFromId, dealEmbedded

				$.ajax({
					method: 'GET',
					url: `/api/v4/leads/${idFromUrl}?with=contacts`,
					dataType: 'json',
					beforeSend: function () {
						replaceTextSelectButton(select_pipeline, 'Загрузка...');
					},
					error: function (xhr, status, error) {
						console.log('error func to GET: ', xhr, status, error);
						replaceTextSelectButton(select_pipeline, 'Ошибка');
					},
					success: function (data) {
						dealFromId = data
						dealEmbedded = dealFromId._embedded
					}
				})

				$('[name="select-pipeline"]').on('change', function () {
					let status_id = $(this).val();
					console.log('status -pipeline- ', status_id);
					if (status_id != '') {
						...
						console.log('statuses (second) --- ', statuses);
				...
				$('.copy-lead').on('change', '[name="select-status"]', function () {
					let change_id = $(this).val();
					console.log('second change id ---- ', change_id);
					if (change_id == '') {
					...
					if (!$(this).hasClass('copy-lead__button_disable')) {
						let pipeline_id = Number($('.copy-lead [name="select-pipeline"]').val()),
							...
							$.ajax({
								...
								url: '/api/v4/leads',
								headers: {
									 'Authorization': `Bearer erEaMW3elnWqULGk9btZ2gV3mCcTlhzSJTJuygRTzljFfbUfmFaBukvR7UFgdndl`,
									 'Content-Type': 'application/json'
								},
								data: JSON.stringify([
									{
										 'name': dealFromId.name + " (Копия)",
										 'price': dealFromId.price,
										 'status_id': status_id,
										 'pipeline_id': pipeline_id,
										 'responsible_user_id': dealFromId.responsible_user_id,
										 '_embedded': {
											  'contacts': dealEmbedded.contacts.map(contact => ({ id: contact.id })),
											  'companies': dealEmbedded.companies ? dealEmbedded.companies.map(company => ({ id: company.id })) : [],
											  'tags': dealEmbedded.tags ? dealEmbedded.tags.map(tag => ({ id: tag.id })) : []
										 }
									}
							  ]),
								beforeSend: function () {
								...
								error: function (xhr, status, error) {
									$('.copy-lead__info')
										...
										.text('Ошибка');
									replaceTextSelectButton(select_pipeline, 'Ошибка');
									console.error('error func to POST: ', xhr, status, error);
								},
								success: function (data) {
									$('.copy-lead__info')
										...
										.text('Готово!');
									 console.log('success func to POST: ', data);
								}
						  });
					}
				});
